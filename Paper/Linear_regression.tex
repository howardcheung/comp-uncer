...
Finding the coefficients of the 10 coefficient maps can be treated as a liniear regression problem, whith the evaporation and condensing temperature and their linear, squared and cubed combinations being the independent variables ${\vec x}$ and the power consumption of the compressor being the estimated dependend variable $y$. To better understand the uncertainty sources of the model, a brief review of linear regression along with how this applies to the polynomial compressor map is shown subsequently.
...
Unfortunately it is not possible to obtain the true model, since all measurement data that is used for training the model has measurement error. Therefore, we need to estimate the model parameters to be able to calculate an estimate of the output. To emphasize the difference between the two models, a hat ( $\hat{}$ ) is used for both, estimated parameters $\hat {\vec \beta}$ and estimated output, $\hat y$,
...
The parameters for the model can be estimated using measuremnt data, for non-weighted reduction of the squares of the errrors it can be shown that the  parmeter vector $\hat \vec \beta$ can be calculated as
...
where the training data input matrix ${{\mathbf{X}}_{train}}$, composed of input data vectors ${\vec x}_{train,i}$ for each data point $i$ is constructed as
...
The accuracy $\sigma$ of the model for the training data points is 
...
where $\beta_i$ are the (estimated) model coefficients and $T_{evap},T_{cond}$ are the evaporation and condensing dew point temperatures. Referring to eqn. \ref{eq:estimted_lin_model}, $\hat {\dot W}$ takes the place of $\hat y$, the vector composed of $\beta_i$ is $\hat {\vec \beta}$, and the vector of independent variables, $\vec x$ is the vector of the independent variables (here: dew point temperatures),
